...
void Skybox::GenerateCubemap(unsigned int textureID)
{
    ...
    generateCubemapShader.setMat4("projection", captureProjection);
    generateCubemapShader.setTexture("equirectangularMap", textureID);

    // set viewport to the same size as capture resolution
    glViewport(0, 0, CaptureResolution, CaptureResolution);

    glBindFramebuffer(GL_FRAMEBUFFER, captureFBO);
    for (unsigned int i = 0; i < 6; ++i)
    {
        generateCubemapShader.setMat4("view", captureViews[i]);
